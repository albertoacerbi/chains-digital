...
# negative <- negative %>%
#  mutate(proportion = replace(proportion, content=="attractive", proportion + 0.05))
...
negative %>%
  ggplot(aes(x = chain_position, y = proportion, colour = content)) + 
    ...
    theme_bw() + 
    ylim(0,1) +
    labs(x = "Chain position", y = "Proportion of propositions transmitted") 
```

Here the output of the models:

```{r, message = FALSE}
no_content_model <- lmer(proportion ~ chain_position + (1|chain_ID), data = negative)
AIC(no_content_model)
content_model <- lmer(proportion ~ content + chain_position + (1|chain_ID), data = negative)
AIC(content_model)
```

As expected, with no difference between content, the AIC of the model without content is lower.
