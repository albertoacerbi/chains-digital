...
See below the code for the analysis and the plots with simulated data (only for one comparison, this will be replicated three time for each attractive content). For the simulated data, I simply assume that proportions are normally distributed (and truncated between 0 and 1). The mean decreases along the chain position (i.e. participants reproduce less sentences) but there is *not* difference between contents (uncomment lines "test with difference" for having it).
